...
xAI serves an OpenAI-compatible API at ``https://api.x.ai/v1``, so we
route through :class:`langchain_openai.ChatOpenAI` with a custom
``base_url``. The provider name (``"grok"``) keys the keychain entry
and the env var (``XAI_API_KEY``).
"""
...
def build(spec: ModelSpec, **kwargs: Any) -> BaseChatModel:
    """Construct a ChatOpenAI instance pointed at xAI's endpoint."""
