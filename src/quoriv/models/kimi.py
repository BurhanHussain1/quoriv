"""Kimi (Moonshot AI) provider backend.

Moonshot serves an OpenAI-compatible API at ``https://api.moonshot.ai/v1``,
so we route through :class:`langchain_openai.ChatOpenAI` with a custom
``base_url``. The provider name (``"kimi"``) keys the keychain entry and
the env var (``MOONSHOT_API_KEY``).
"""
...
def build(spec: ModelSpec, **kwargs: Any) -> BaseChatModel:
    """Construct a ChatOpenAI instance pointed at Moonshot's endpoint."""
